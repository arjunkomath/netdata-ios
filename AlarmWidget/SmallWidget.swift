...
struct SmallWidget_Previews: PreviewProvider {
    static var previews: some View {
        SmallWidget(entry: AlarmsEntry(count: 0, criticalCount: 0, alarms: [:], date: Date()))
            .previewContext(WidgetPreviewContext(family: .systemSmall))
        
        SmallWidget(entry: AlarmsEntry(count: 1, criticalCount: 0, alarms: [:], date: Date()))
            .previewContext(WidgetPreviewContext(family: .systemSmall))
        
        SmallWidget(entry: AlarmsEntry(count: 2, criticalCount: 1, alarms: [:], date: Date()))
            .previewContext(WidgetPreviewContext(family: .systemSmall))
    }
}
